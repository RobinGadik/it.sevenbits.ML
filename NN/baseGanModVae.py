...
class discriminator(nn.Module):
    ...
    def __init__(self, input_size=32, n_class=10):
        ...
        #self.deconv1 = nn.ConvTranspose2d( 20 , 10 , kernel_size = 3)
        self.deconv2 = nn.ConvTranspose2d( 10 , 1, kernel_size = 3)
        
        print(self.conv1)
        print(self.deconv2)
        print(input_size)
        
        self.fc1 = nn.Linear(15*15, 1024)
        print(self.fc1.out_features)
        self.fc2 = nn.Linear(self.fc1.out_features, 512)
    ...
    def forward(self, x):
        ...
        '''
        print("X AFTER CONV 1 VIEW in forward sizes ")
        print(x.size()[0])
        print(x.size()[1])
        print(x.size()[2])
        print(x.size()[3])
        ...
        '''
        print("X AFTER CONV 2 VIEW in forward sizes ")
        print(x.size()[0])
        print(x.size()[1])
        print(x.size()[2])
        print(x.size()[3])
        '''
        #x = F.relu(self.deconv1(x), 2)
        x = F.relu(self.deconv2(x), 2)
        '''
        print("X AFTER DECONV 1,2 VIEW in forward sizes ")
        print(x.size()[0])
        print(x.size()[1])
        print(x.size()[2])
        print(x.size()[3])
...
for epoch in range(train_epoch):
    ...
    for x_, _ in train_loader:
        ...
        #print("x_ sizes is %d x %d x %d x %d " % ( x_.size()[0], x_.size()[1] , x_.size()[2] , x_.size()[3]) )
        x_ = x_.view(-1, 28 * 28) ##resize to 128x762    
        #print("x_.VIEW sizes is %d x %d  " % ( x_.size()[0], x_.size()[1]  ) )
